...
object LocalResultEventBus {
  ...
  val current: ResultEventBus
    @Composable get() = LocalResultEventBus.current ?: error("No ResultEventBus has been provided")
...
class ResultEventBus {
  ...
  val channelMap = mutableStateMapOf<String, Channel<NavResult?>>()
  ...
  inline fun <reified T> sendResult(resultKey: String = T::class.toString(), result: NavResult) {
